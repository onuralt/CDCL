...
static Clause *add_clause(std::vector<int> &literals) {
  ...
  if (size != 1) {
    c->litwatch_a = c->literals[0];
    c->litwatch_b = c->literals[size - 1];
  }
...
static bool watchUpdate(Clause *c) {
  // Cache the values of litwatch_a and litwatch_b for faster access
  const int value_a = values[c->litwatch_a];
  const int value_b = values[c->litwatch_b];
  // Iterate through each literal in the clause
  for (const auto &lit : *c) {
    // Cache the value of the current literal for faster access
    const int value_lit = values[lit];
    // Check if the literal is unassigned (value_lit = 0)
    if (value_lit == 0) {
      // Check if litwatch_a is unassigned and different from litwatch_b
      if (value_a < 0 && lit != c->litwatch_b) {
        c->litwatch_a = lit; // Update litwatch_a to lit
        return true;         // Return true, indicating a watch update occurred
      }
      // Check if litwatch_b is unassigned and different from litwatch_a
      if (value_b < 0 && lit != c->litwatch_a) {
        c->litwatch_b = lit; // Update litwatch_b to lit
        return true;         // Return true, indicating a watch update occurred
      }
    }
    // Check if the literal is satisfied (value_lit = 1)
    else if (value_lit == 1) {
      return true; // Return true, indicating the clause is satisfied
    }
  }
  // Check if either litwatch_a or litwatch_b is false assigned
  if (value_a < 0 || value_b < 0) {
    return false; // Return false, indicating a conflict occurred
  }
  return true; // Return true, indicating no watch update occurred
}
...
static Clause *propagate(void) {
  while (propagated != assigned) {
    ...
    // Get the occurrences of the negation of the literal
    ...
    for (auto c : occurrences) {
      ...
      // Check if either litwatch_a or litwatch_b is already satisfied (assigned
      // value = 1)
      if (values[c->litwatch_a] == 1 || values[c->litwatch_b] == 1)
        continue; // Skip to the next occurrence

      // Check if either litwatch_a or litwatch_b is unassigned (assigned value
      // < 0)
      if (values[c->litwatch_a] < 0 || values[c->litwatch_b] < 0) {
        // Check if there is a watch update or propagation is not possible
        if (!watchUpdate(c)) {
          // Determine the literal that can be propagated
          if (values[c->litwatch_a] == 0)
            unit = c->litwatch_a;
          else if (values[c->litwatch_b] == 0)
            unit = c->litwatch_b;

          // Check if there is a conflict (no unit literal found)
          if (!unit) {
            conflicts++;
            resConf++;
            debug(c, "conflicting");
            return c; // Return the conflicting clause
          }

          debug(c, "forced %s by", debug(unit));
          assign(unit, c); // Assign the unit literal and update the clause
        }
        continue; // Skip to the next occurrence
      }
    }
  }
  return 0; // Return 0, indicating no conflicting clause was found
}

void reluctantDoubling(uint64_t &u, uint64_t &v) {
  if ((u & -u) == v) {
    u++;
    v = 1;
  } else {
  ...
  // Push u and v into the global vector
  uv[0] = u;
  uv[1] = v;
}
...
static bool restart() {
  int n = uv[idx];
  size_t limit = 256 * (1 << n);
  if (resConf != limit)
    return false;
  idx++;
  if (idx > 1) {
    idx = 0;
    reluctantDoubling(u, v);
  }
  backtrack(0);
  resConf = 0;
  return true;
...
static void decide(void) {
  ...
  // Used for obtaining random numbers from the hardware or operating system.
  std::random_device rd;
  // Produce random numbers using the Mersenne Twister algorithm.
  std::mt19937 generator(rd());
  // Generate random integers within the range of 1 to n.
  std::uniform_int_distribution<int> distribution(1, variables);

  do {
    // Produces a random integer within the specified range.
    lit = distribution(generator);
  } while (values[lit] != 0);
  level++;
  debug("decide %d", lit);
